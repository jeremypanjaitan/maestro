 ...
 * period, status), a table of items (No, Tanggal, Murid, Jam, Tipe,
 * Pertemuan ke-N, Rate), and a total row. Node-only (exceljs) — only ever
 * import this from a route handler or another server-only module.
 */
export async function buildPayrollWorkbook(payroll: PayrollExportData): Promise<Buffer> {
  ...
  const headerRow = sheet.addRow(["No", "Tanggal", "Murid", "Jam", "Tipe", "Pertemuan ke-N", "Rate"]);
  ...
  payroll.items.forEach((item, index) => {
    sheet.addRow([
      ...
      item.studentName,
      item.startTime,
      CLASS_TYPE_LABELS[item.classType],
